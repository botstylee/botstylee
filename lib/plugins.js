...
var __dirname = Helper.__dirname(
	import.meta)
var pluginFolder = Helper.__dirname(join(__dirname, '../plugins/index'))
var pluginFilter = filename => /\.(mc)?cjs$/.test(filename)

// inspired from https://github.com/Nurutomo/mahbod/blob/main/src/util/PluginManager.ts

var watcher, plugins, pluginFolders = []
watcher = plugins = {}
...
async function filesInit(pluginFolder = pluginFolder, pluginFilter = pluginFilter, conn) {
	...
	await Promise.all(readdirSync(folder).filter(pluginFilter).map(async filename => {
		try {
			var file = global.__filename(join(folder, filename))
			var module = await import(file)
	...
	var watching = watch(folder, reload.bind(null, conn, folder, pluginFilter))
...
async function reload(conn, pluginFolder = pluginFolder, pluginFilter = pluginFilter, _ev, filename) {
	if (pluginFilter(filename)) {
		var dir = global.__filename(join(pluginFolder, filename), true)
		...
		var err = syntaxerror(readFileSync(dir), filename, {
			sourceType: 'module',
			allowAwaitOutsideFunction: true
		})
		if (err) conn.logger.error(`syntax error while loading '${filename}'\n${format(err)}`)
		else try {
			var module = await importFile(global.__filename(dir)).catch(console.error)
