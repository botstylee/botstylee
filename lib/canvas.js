...
export function levelup(teks, level) {
	return new Promise(async (resolve, reject) => {
		...
		var font = join(__dirname, '../src/font')
		var fontLevel = join(font, './level_c.otf')
		var fontTexts = join(font, './texts.otf')
		var xtsx = join(__dirname, '../src/lvlup_template.jpg')
		...
		var [_spawnprocess, ..._spawnargs] = [...(global.support.gm ? ['gm'] : global.support.magick ? ['magick'] : []),
			'convert',
			xtsx,
			'-font',
			fontTexts,
