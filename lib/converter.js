...
function ffmpeg(buffer, args = [], ext = '', ext2 = '') {
	return new Promise(async (resolve, reject) => {
		try {
			var tmp = join(__dirname, `../tmp/${Date.now()}.${ext}`)
			var out = `${tmp}.${ext2}`

			var isStream = Helper.isReadableStream(buffer)
			...
			spawn('ffmpeg', [
				...
				.once('close', async (code) => {
					try {
						...
						var data = createReadStream(out)
						resolve({
							data,
							filename: out,
							async toBuffer() {
								var buffers = []
								for await (var chunk of data) buffers.push(chunk)
