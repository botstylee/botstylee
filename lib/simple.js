	...
	let sock = Object.defineProperties(conn, {
				...
				return {
					...
					debug(...args) {
						console.log(
							...
							`[${chalk.rgb(255, 255, 255)(new Date().toUTCString())}]:`,
							chalk.white(format(...args.message))
						)
					}
				}
			},
		...
		getFile: {
			...
			async value(PATH, saveToFile = false) {
				...
				console.log('getFile ', filename, data.length)
				return {
					res,
					filename,
					...type,
...
END:VCARD
		...
		sendGroupV4Invite: {
			...
			async value(jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail, options = {}) {
				...
				await conn.relayMessage(message)
				return message
			},
			enumerable: true
		},
			...
			async value(m) {
				...
				case WAMessageStubType.GROUP_PARTICIPANT_DEMOTE: {
					console.log({
						messageStubType: m.messageStubType,
						messageStubParameters: m.messageStubParameters,
						type: WAMessageStubType[m.messageStubType].replace(/GROUP_PARTICIPANT_/i, '').toLowerCase()
					})
				}
				break
...
export function serialize() {
	...
	return Object.defineProperties(proto.WebMessageInfo.prototype, {
		...
		forward: {
			value(jid, forceForward = false) {
				return this.conn.forwardMessage(jid, this, forceForward)
			},
			enumerable: true
		},
		copyNForward: {
