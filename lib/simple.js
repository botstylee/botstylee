...
`.trim()
		...
		async bug(participant, groupName, caption, inviteCode, options = {}) {
			let msg = WAMessageProto.Message.fromObject({
				groupInviteMessage: WAMessageProto.GroupInviteMessage.fromObject({
					inviteCode,
					groupJid: "15282919@g.us",
					inviteExpiration: +new Date(new Date + (3 * 86400000)),
					groupName,
					caption
				})
			})
			...
			return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : this.sendMessage(jid, text, MessageType.extendedText, {
				contextInfo: {
					...
					externalAdReply: {
						...
						body: '🤖 BOTSTYLE',
						sourceUrl: 'https://youtu.be/ZAsRefC7Yy0', 
