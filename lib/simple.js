	...
	var sock = Object.defineProperties(conn, {
			...
			getFile: {
				...
				async value(PATH, saveToFile = false) {
					...
					var name = res ? res.headers ? res.headers.get("content-disposition") ? res.headers.get('content-disposition').split("filename=")[1].replaceAll(/(\")/g, "") : Date.now() + '.' + streamWithType.fileType.ext : Date.now() + '.' + streamWithType.fileType.ext : Date.now() + '.' + streamWithType.fileType.ext
					log(name)
					if (data && saveToFile && !filename) {
						filename = path.join(__dirname, `../tmp/${name}`)
			...
			sendFile: {
				...
				async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
					...
					var rw
					if (file.res !== null) {
						rw = file.res.headers.get('content-type')
					} else {
						rw = file.mime
					}
					if (/webp/.test(file.mime) || (/image/.test(type.mime) && options.asSticker)) mtype = 'sticker'
