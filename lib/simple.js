...
async sendSticker(jid, media, quoted, opts = {}) {
   ...
   return await this.sendMessage(jid, await convert(medias.data, null, opts.packname || global.packname, opts.author || global.author), 'stickerMessage', { quoted: quoted, ...opts })
    ...
    reply(jid, text, quoted, options) {
      return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : this.sendMessage(jid, text, MessageType.extendedText, { contextInfo: { mentionedJid: this.parseMention(text),  externalAdReply: {title: 'ꜱɪᴍᴩʟᴇ ᴡʜᴀᴛꜱᴀᴩᴩ ʙᴏᴛ', body: '© Anti', sourceUrl: '', thumbnail: global.thumb3}}, sendEphemeral: true, quoted, ...options })
    }
    /**
     * Fake Replies
     * @param {String} jid
