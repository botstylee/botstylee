...
var delay = ms => isNumber(ms) && new Promise(resolve => setTimeout(resolve, ms))

var QUEQUE_DELAY = 5 * 1000
...
export default class Queque extends EventEmitter {
	...
	waitQueue(item) {
		return new Promise((resolve, reject) => {
			...
			if (this.has(item)) {
				var solve = async (removeQueque = false) => {
