...
function bind(conn) {
	...
	function updateNameToDb(contacts) {
		...
		try {
			...
			console.log(`Process ${contacts.length} pushName`)
	...
	conn.ev.on('chats.set', async ({
		...
		try {
			console.log(`Process ${chats.length} chats.set`)
			for (let {
					id,
					name,
					readOnly
	...
	conn.ev.on('group-participants.update', async function updateParticipantsToDb({
		...
		console.log(`Process group (${id}) update`)
	...
	conn.ev.on('groups.update', async function groupUpdatePushToDb(groupsUpdates) {
		try {
			console.log(`Process ${groupsUpdates.length} groups Update`)
	...
	conn.ev.on('chats.upsert', function chatsUpsertPushToDb(chatsUpsert) {
		try {
			...
			console.log(`Process chats.upsert (${id})`)
			conn.chats[id] = {
				...(conn.chats[id] || {}),
				...chatsUpsert,
				isChats: true
	...
	conn.ev.on('presence.update', async function presenceUpdatePushToDb({
		...
		try {
			console.log(`Process presence.update (${id})`)
