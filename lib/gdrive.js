...
// time.
var TOKEN_PATH = join(__dirname, '..', 'token.json')
...
class GoogleAuth extends EventEmitter {
	...
	async authorize(credentials) {
		...
		var oAuth2Client = new google.auth.OAuth2(client_id, client_secret, `http://localhost:${port}`)
		try {
			...
			var code = await promisify(this.once).bind(this)('token')
