...
handler.before = async function (m) {
    ...
    if (/^nyerah$/i.test(m.text)) {
        await this.sendButton(m.chat, `Jawabannya adalah ${JSON.parse(JSON.stringify(this.asahotak[id][1].jawaban))}`.trim(), 'BOTSTYLE', 'ASAH OTAK', '.asahotak').then(() => { delete this.asahotak[id] })
    }
