...
handler.before = async function (m) {
    ...
    if (/^(me)?nyerah$/i.test(m.text)) {
        await this.sendButton(m.chat, `Jawabannya adalah ${JSON.parse(JSON.stringify(this.caklontong[id][1].jawaban))}`.trim(), 'BOTSTYLE', 'CAK LONTONG', '.caklontong').then(() => { delete this.caklontong[id] })
    }
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
        global.DATABASE.data.users[m.sender].exp += this.caklontong[id][2]
