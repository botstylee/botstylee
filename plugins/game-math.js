...
`.trim()
	...
	conn.math[id] = [
		await conn.reply(m.chat, `Berapa hasil dari *${math.str}*?\n\nTimeout: ${(math.time / 1000).toFixed(2)} detik\nBonus Jawaban Benar: ${math.bonus} MP`, m),
		math, 4,
		setTimeout(async () => {
			if (conn.math[id]) await conn.reply(m.chat, `Waktu habis!\nJawabannya adalah ${math.result}`, conn.math[id][0])
			delete conn.math[id]
		}, math.time)
	]
}
...
var modes = {
	...
	extreme: [-9999, 9999, -9999, 9999, '*/', 80000, 7000],
	impossible: [-999999, 999999, -999999, 999999, '*/', 100000, 12000],
	impossible2: [-9999999, 9999999, -9999, 9999, '/', 120000, 15000]
}
