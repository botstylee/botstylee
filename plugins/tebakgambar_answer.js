...
handler.before = async function (m) {
  ...
  if (/^nyerah$/i.test(m.text)) {
    await this.sendButton(m.chat, `Jawabannya adalah ${JSON.parse(JSON.stringify(this.tebakgambar[id][1].jawaban))}`.trim(), 'BOTSTYLE', 'TEBAK GAMBAR', '.tebakgambar').then(() => { delete this.tebakgambar[id] })
  }
  ...
  if (m.text.toLowerCase() == json.jawaban.toLowerCase()) {
    ...
    await this.sendButton(m.chat, `*Benar!* +${this.tebakgambar[id][2]} XP`, 'BOTSTYLE', 'TEBAK GAMBAR', '.tebakgambar')
