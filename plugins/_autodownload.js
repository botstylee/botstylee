...
handler.all = async function (m, { isPrems, isOwner }) {
    ...
    if (/^.*cocofun/i.test(m.text)) {
        ...
        await this.sendVideo(m.chat, json.download, `*BOTSTYLE*`, m, { thumbnail: buf })
    }
    ...
    if (/^.*(fb.watch|facebook.com)/i.test(m.text)) {
        facebook(m.text).then(async res => {
            ...
            await this.sendVideo(m.chat, json.data[1].url, '*BOTSTYLE*', m, buf)
        }).catch(_ => _)
    }
    ...
    if (/^.*(pinterest.com\/pin|pin.it)/i.test(m.text)) {
        pin(m.text).then(async res => {
            ...
            await this.sendVideo(m.chat, json.data.url, `*BOTSTYLE*`, m, { thumbnail: buf })
        }).catch(_ => _)
    }
