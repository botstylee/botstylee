...
handler.before = async function (m) {
    ...
    if (/^nyerah$/i.test(m.text)) {
        await this.sendButton(m.chat, `Jawabannya adalah ${JSON.parse(JSON.stringify(this.tebakkata[id][1].jawaban))}`.trim(), 'BOTSTYLE', 'TEBAK KATA', '.tebakkata').then(() => {
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
        ...
        await this.sendButton(m.chat, `*Benar!* +${this.tebakkata[id][2]} XP`, 'BOTSTYLE', 'TEBAK KATA', '.tebakkata')
