...
if (global.conns instanceof Array) console.log() // for (var i of global.conns) global.conns[i] && global.conns[i].user ? global.conns[i].close().then(() => delete global.conns[id] && global.conns.splice(i, 1)).catch(global.conn.logger.error) : delete global.conns[i] && global.conns.splice(i, 1)
else global.conns = []
...
var handler = async (m, {
	...
	var parent = args[0] && args[0] == 'plz' ? conn : global.conn
	var auth = false
	if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
		var id = global.conns.length
		var conn = new global.conn.constructor()
		var state, saveState
		conn.browserDescription = ['Anti-bot', '', '5.0']
		if (args[0] && args[0].length > 200) {
			var json = Buffer.from(args[0], 'base64').toString('utf-8')
		...
		conn.ev.on('connection.update', async ({
			...
			var scan = await parent.sendFile(m.chat, await qrcode.toDataURL(qr, {
				scale: 8
		...
		setTimeout(() => {
			if (conn.user) return
			conn.close()
			var i = global.conns.indexOf(conn)
			if (i < 0) return
		...
		conn.ev.on('connection.update', (update) => {
			...
			if (connection === 'close') {
				var shouldReconnect = (lastDisconnect.error as Boom)?.output?.statusCode !== DisconnectReason.loggedOut
