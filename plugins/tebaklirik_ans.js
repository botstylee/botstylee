...
handler.before = async function (m) {
    ...
    if (/^nyerah$/i.test(m.text)) {
        await this.sendButton(m.chat, `Jawabannya adalah ${JSON.parse(JSON.stringify(this.tebaklirik[id][1].jawaban))}`.trim(), 'BOTSTYLE', 'TEBAK LIRIK', '.tebaklirik').then(() => {
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
        global.DATABASE.data.users[m.sender].exp += this.tebaklirik[id][2]
