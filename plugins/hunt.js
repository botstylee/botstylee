let handler = async (m, { conn, text }) => {
	...
	if (new Date -  global.DATABASE.data.users[m.sender].lasthunt > 120000) {
		...
		let pesan = `*${pname}* Menemukan Dan Membunuh *${monsterName}*\nMendapatkan ${new Intl.NumberFormat('en-US').format(coins)} 📀coins & ${new Intl.NumberFormat('en-US').format(exp)} 🎗️XP\nBerkurang -${dmg}💉Hp, Tersisa ${player.healt}/${100}`
		m.reply(pesan)
